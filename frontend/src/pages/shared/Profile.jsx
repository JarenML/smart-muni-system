...
import { User, Mail, Phone, Key, Save, Contact } from "lucide-react";
...
const Profile = () => {
  ...
  // FIX: Inicializar con valores por defecto para evitar undefined
  ...
  useEffect(() => {
    if (user) {
      // FIX: Garantizar que todos los valores sean strings
  ...
  const handleProfileChange = (e) => {
    ...
    setProfile((prev) => ({ ...prev, [name]: value || "" })); // FIX: Garantizar string
    if (errors[name]) setErrors((prev) => ({ ...prev, [name]: null }));
  };

  const handlePasswordChange = (e) => {
    const { name, value } = e.target;
    setPassword((prev) => ({ ...prev, [name]: value || "" })); // FIX: Garantizar string
    if (errors[name]) setErrors((prev) => ({ ...prev, [name]: null }));
  };
